#!/usr/bin/env python
"""
Online DiffusionDPO for FLUX.1-dev with full-weight finetuning + HPSv2 AI feedback.
...
Reference model is a separate frozen copy of the transformer, FSDP-sharded
to fit in GPU memory.

Usage:
    torchrun --nproc_per_node=4 train_flux_dpo_full.py --pretrained_model_name_or_path ...
"""
...
# ---------------------------------------------------------------------------
# FLUX model loading (full-weight finetuning, separate reference model)
# ---------------------------------------------------------------------------

def load_flux_models(args, device):
    """Load FLUX policy transformer, reference transformer, VAE, and scheduler.

    Policy transformer: all parameters trainable.
    Reference transformer: separate frozen copy for DPO reference predictions.
    """
    import gc
    ...
    # Load a separate reference transformer (frozen copy for DPO)
    logger.info("Loading reference transformer...")
    ...
    # Policy transformer: make all parameters trainable
    transformer.requires_grad_(True)

    # VAE stays frozen on device
    vae.requires_grad_(False)
    vae = vae.to(device)
    vae.eval()

    return transformer, ref_transformer, vae, scheduler
...
def setup_fsdp(transformer, ref_transformer, device, args):
    """Wrap both policy and reference transformers with FSDP for distributed training.

    The reference transformer (~24GB in bf16) is too large to replicate per GPU,
    so it must also be FSDP-sharded even though it's frozen.
    """
    ...
    return transformer, ref_transformer
...
# ---------------------------------------------------------------------------
# DPO training step with separate reference model
# ---------------------------------------------------------------------------

def dpo_training_step(transformer, ref_transformer, vae, batch, latents_w, latents_l, args, device):
    """
    Compute flow-matching DPO loss.

    1. Sample random sigma, create noisy versions of preferred/unpreferred latents
    2. Predict velocity with policy transformer and frozen reference transformer
    ...
    # --- Reference prediction (frozen ref_transformer, no grad) ---
    with torch.no_grad():
        ref_pred_w = ref_transformer(
    ...
    # --- Policy prediction (trainable transformer, with grad) ---
...
def save_full_checkpoint(transformer, optimizer, step, output_dir):
    """Save full transformer weights using FSDP full state dict gathering."""
    from torch.distributed.fsdp import (
        FullyShardedDataParallel as FSDP,
        StateDictType,
        FullStateDictConfig,
    ...
    if dist.get_rank() == 0:
        torch.save(state_dict, os.path.join(save_dir, "transformer_weights.pt"))
...
def parse_args():
    parser = argparse.ArgumentParser(description="Online DiffusionDPO for FLUX.1-dev with full-weight finetuning")
...
def main():
    ...
    # --- Load models ---
    transformer, ref_transformer, vae, scheduler = load_flux_models(args, device)
    ...
    # --- FSDP ---
    transformer, ref_transformer = setup_fsdp(transformer, ref_transformer, device, args)
    ...
    while global_step < args.max_train_steps:
        ...
        for batch in dataloader:
            ...
            loss, metrics = dpo_training_step(
                transformer, ref_transformer, vae, batch, latents_w, latents_l, args, device,
            )
